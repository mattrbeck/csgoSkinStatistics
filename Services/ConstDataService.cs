namespace CSGOSkinAPI.Services
{
    public class ConstDataService
    {
        ...
        internal ConstDataService(string dataDirectory)
        {
            ...
            var stickerJson = File.ReadAllText(Path.Combine(dataDirectory, "stickers.json"));
            ...
            var skinImageJson = File.ReadAllText(Path.Combine(dataDirectory, "skin-images.json"));
            ...
            var blueGemPath = Path.Combine(dataDirectory, "blue-gem.json");
            _blueGem = File.Exists(blueGemPath)
                ? JsonSerializer.Deserialize<Dictionary<string, Dictionary<string, Dictionary<string, BlueGemPattern>>>>(
                      File.ReadAllText(blueGemPath), JsonOptions) ?? []
                : [];
            ...
            var fadePath = Path.Combine(dataDirectory, "fade.json");
            _fade = File.Exists(fadePath)
                ? JsonSerializer.Deserialize<Dictionary<string, Dictionary<string, double[]>>>(
                      File.ReadAllText(fadePath), JsonOptions) ?? []
                : [];
        }
